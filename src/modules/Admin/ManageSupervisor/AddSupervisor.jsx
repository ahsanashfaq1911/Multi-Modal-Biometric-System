  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(
        "http://127.0.0.1:5000/add_supervisor",
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
      );
      setSnackbarMsg(res.data.message || "Supervisor added!");
      ...
      const msg = err.response?.data?.error || "Failed to add supervisor.";
      setSnackbarMsg(msg);
      setSnackbarSeverity("error");
      setOpenSnackbar(true);
    }
  ...
  return (
    <AppLayout>
      <CustomBox sx={{ maxWidth: 600, mx: "auto", p: 3 }}>
        ...
        {/* Submit Button */}
        <Box textAlign="right">
          <CustomButton onClick={handleSubmit}>Save Supervisor</CustomButton>
        </Box>
